            ...
            {ACTIVITIES.map(a => (
              <div key={a.name} style={{ display: 'flex', gap: 16, background: 'var(--bg-card)', border: '1px solid var(--border)', padding: 20, borderRadius: 'var(--radius-lg)' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <h3 style={{ fontSize: 17, fontWeight: 800, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.name}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 300px), 1fr))', gap: 16 }}>
            {STAYS.map(s => (
              <div key={s.name} className="glass-panel" style={{ padding: 28, borderRadius: 'var(--radius-lg)', display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <a href={s.maps} target="_blank" style={{ textAlign: 'center', padding: '12px 0', background: 'var(--accent-dim)', color: 'var(--accent)', fontWeight: 800, borderRadius: 10, textDecoration: 'none', fontSize: 13, border: '1px solid var(--accent-border)' }}>LOCATE</a>
              </div>
            ))}
          </div>
        </section>
